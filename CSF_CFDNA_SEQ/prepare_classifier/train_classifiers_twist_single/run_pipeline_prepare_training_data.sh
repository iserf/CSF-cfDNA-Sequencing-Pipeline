...
script_dir=/home/iser/cfDNA_docker/pipeline_v2/prepare_classifier/train_classifiers_twist_single
...
date | tee -a $LOG_FILE
$script_dir/somaticseq_callers.sh $ID_T ${ID_T}.aligned.deduped.bam $training $BED 2>>$ERROR_FILE 1>>$LOG_FILE
wait
...
date | tee -a $LOG_FILE
$script_dir/octopus.sh $ID_T ${ID_T}.aligned.deduped.bam $training $BED 2>>$ERROR_FILE 1>>$LOG_FILE
wait
...
date | tee -a $LOG_FILE
$script_dir/prepare_tsv.sh $ID_T ${ID_T}.aligned.deduped.bam $training $SNV_truth $Indel_truth $BED 2>>$ERROR_FILE 1>>$LOG_FILE
wait

##Var calling finished##
date | tee -a $LOG_FILE
echo "Samples ${ID_T} SINGEL MODE: .tsv for classifier training are created under $work_dir/tsv" | tee -a $LOG_FILE
