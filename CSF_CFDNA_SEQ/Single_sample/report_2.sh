...
##Scripts
docker run --rm -v ${home_dir}:/home/iser/cfDNA_docker -u $UID --memory 200G 31071993/cfdna_pipeline_v2:pipeline_v2_report /bin/bash -c \
"python /mydata/preprocess_SSeq_vcf_single.py $SNV_raw ${SNV_raw}.preprocessed.vcf"

docker run --rm -v ${home_dir}:/home/iser/cfDNA_docker -u $UID --memory 200G 31071993/cfdna_pipeline_v2:pipeline_v2_report /bin/bash -c \
"python /mydata/preprocess_SSeq_vcf_single.py $INDEL_raw ${INDEL_raw}.preprocessed.vcf"
...
"python /mydata/create_report_single.py $SNV $INDEL $SNV_clinvar $Indel_clinvar $special_positions_raw $special_positions_deduped $report"
